...
var connectionString = builder.Configuration.GetConnectionString("BlogsUsersDb");
builder.Services.AddDbContext<BlogsUsersDb>(options => options.UseSqlite(connectionString));
builder.Services.AddMediatR(config => config.RegisterServicesFromAssembly(typeof(BlogsUsersDb).Assembly));
